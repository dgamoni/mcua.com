 ...
 */
    function tfuse_shortcode_posts($args = null)
    {
        ...
        if ($post_type == 'cases')
        {
            $query = new WP_Query( array ( 'post_type' => 'cases', 'orderby' => 'post_date',  'order '=>'DESC', 'posts_per_page'=>$items ) );
            ...
            if( !empty($posts) )
            {
                ...
                foreach($posts as $post_elm)
                {
                    ...
                    $excerpt = $post_elm->post_excerpt;

                    $tf_post_option[$count]['excerpt']         =  $excerpt;
                    $tf_post_option[$count]['mini_excerpt'] = tfuse_shorten_string($excerpt,20);

                    //$tf_post_option[$count]['content']         =  get_post_field('post_content', $post_elm->ID);
                    $tf_post_option[$count]['content']         =  $post_elm->post_content;

                    $tf_post_option[$count]['charges'] = tfuse_page_options('charges', null, $post_elm->ID );
                    $tf_post_option[$count]['verdict'] = tfuse_page_options('verdict', null, $post_elm->ID );

                    $tf_post_option[$count]['photo_title'] = tfuse_page_options('title', null, $post_elm->ID );
                    $tf_post_option[$count]['photo_src'] = tfuse_page_options('thumbnail_image',null, $post_elm->ID);
                    if($tf_post_option[$count]['photo_src'] == null) tfuse_page_options('single_image',null, $post_elm->ID);
                    $count++;
                }
                wp_reset_postdata();
            }
        ...
        if( !empty($posts) )
        {
            ...
            foreach($posts as $post_elm)
            {
                ...
                $tf_post_option[$count]['post_loveit_number']   =  get_post_meta($post_elm->ID,'tfuse_love_it', true);

                $excerpt = ($post_elm->post_excerpt) ? $post_elm->post_excerpt : $post_elm->post_content;
                $position = strpos($excerpt,'...&nbsp;<a href="');
                if ($position) $excerpt = substr($excerpt,0,(strlen($excerpt) - (strlen($excerpt) - $position)));

                $tf_post_option[$count]['post_excerpt']         =  $excerpt;

                $tf_post_option[$count]['mini_post_excerpt'] = tfuse_shorten_string($excerpt,20);
                $count++;
            }
            wp_reset_postdata();
        }
