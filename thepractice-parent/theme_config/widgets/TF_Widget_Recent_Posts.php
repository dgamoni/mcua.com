...
class TFuse_Recent_Posts extends WP_Widget {
    ...
    function TFuse_Recent_Posts() {
        ...
        parent::WP_Widget(false, __('TFuse - Recent Articles', 'tfuse'),$widget_ops);
    }

    function widget($args, $instance) {
        extract( $args );
        $template_directory = get_template_directory_uri() . '/';
        $title = apply_filters('widget_title', empty($instance['title']) ? __('RECENT ARTICLES') : $instance['title'], $instance, $this->id_base);
        $number = esc_attr($instance['number']);
        if ($number>0) { $number = $number>25 ? 25 : $number; } else $number = 8;
    ?>

    <div class="widget-container widget_recent_entries">
        <img src="<?php echo $template_directory; ?>images/icons/widget_icon_01.png" alt="" class="widget_icon">
        <h3><?php echo tfuse_qtranslate($title); ?></h3>
        <ul>
            <?php
