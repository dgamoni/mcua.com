...
function tf_reservationform_shortcode($atts){
    ...
    if($form_exists){
        ...
        $out.='<div class="comment-form"><div id="form_messages" class="submit_message" ></div><form id="reservationForm" action="" method="post" class="reservationForm" name="reservationForm">';
        ...
        foreach($form['input'] as $form_input){
            ...
            if ($is_preview && $input['type'] == 'text' && ($form_input['type']=='7' || $form_input['type']=='8') ) $form_input['ewidthpx'] +=20;
        ...
        $out.='<div class="clear"></div><div class="field_submit" style="clear:left;width:100%; height:50px;">';
        $out.='<a class="link-reset" onclick="resetFields(this,event)" href="#">'.__(urldecode($form['reset_button']),'tfuse').'</a>';
        $out.='<a href="#" id="sending" class="btn-submit" style="display: none; width:170px;"><span>'.__('Sending ...','tfuse').'</span></a>
        <input type="submit" id="send_reservation" class="btn-submit" name="submit" title="Submit mesage" value="'. __(urldecode($form['submit_mess']),'tfuse').'" />
        <img id="sending_img" src="'.$surse.'" alt="Sending" style="margin-right:-235px; margin-top:17px; display: none; float: right;" />
        </div></form></div></div>';
    } else {
...
function res_textarea($input,$form_input){
    return "<div class='row field_textarea alignleft infieldlabel ".$form_input['classes']."' style='".$form_input['floating']."'>
                <label class='textarea' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
